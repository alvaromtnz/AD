<?php
require_once("../dbutils.php");
$db = conectarDB();
$player = null;

if(isset($_POST['buscar'])) {
    $player = getPlayerPorId($db, $_POST['id']);
}

if(isset($_POST['borrar'])) {
    borrarPlayer($db, $_POST['id']);
    echo "<p>Player borrado correctamente</p>";
    $player = null;
}
?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8">
    <title>Borrar Player</title>
</head>
<body>
    <h1>Borrar Player</h1>
    <p><a href="listar.php">Volver a la lista</a></p>
    
    <form method="post">
        <p>ID: <input type="number" name="id" required></p>
        <p><button type="submit" name="buscar">Buscar</button></p>
    </form>
    
    <?php if($player) {?>
    <hr>
    <p>Player encontrado: <?php echo $player['ID']; ?> - <?php echo $player['NOMBRE']; ?> (<?php echo $player['EMAIL']; ?>)</p>
    <form method="post">
        <input type="hidden" name="id" value="<?php echo $player['ID']; ?>">
        <p><button type="submit" name="borrar" onclick="return confirm('¿Seguro?')">Borrar</button></p>
    </form>
    <?php } ?>
</body>
</html>
